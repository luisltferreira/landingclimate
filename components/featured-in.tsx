...
export default function FeaturedIn() {
  ...
  return (
    <section className="py-4 sm:py-6 md:py-8 px-4 sm:px-6 bg-[#191919]">
      <div className="max-w-6xl mx-auto" ref={ref}>
        <p className={`text-center text-xs sm:text-sm font-semibold text-[#d5ffa1] uppercase tracking-wider mb-3 sm:mb-4 transition-all duration-700 ${isRevealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {t.featuredIn.title}
        </p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
          {t.featuredIn.logos && t.featuredIn.logos.length > 0 ? (
            t.featuredIn.logos.map((logo, index) => (
              ...
              >
                {logo.src ? (
                  ...
                  <div className="text-white text-xs sm:text-sm font-semibold text-center px-2 sm:px-4 transition-colors duration-300 hover:text-[#d5ffa1]">
                    {logo.name}
                  </div>
                )}
              </div>
            ...
            <p className="text-white text-sm">Nenhum logo disponível</p>
          )}
        </div>
      </div>
    </section>
